module ApplicationDecorator
  class BookDecorator
    ...
    def self.get_book_attr(isbn, attr)
      add_book_to_redis(isbn, api_book(isbn)) if REDIS.hlen("isbn_#{isbn}").zero?
    ...
    def self.api_book(isbn)
      GoogleBooks.search("isbn:#{isbn}", :api_key => API_KEY).first
    end

    private

    def self.add_book_to_redis(key, book)
      if !book.nil?
